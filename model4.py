...
class RedNet(object):
    ...
    def formNet(self, img_ph, ann_ph, base_filter_num=16):
        conv_layer_list = {}
        deconv_layer_list = {}
        curr_layer = img_ph
        ...
        for i in range(4, -1, -1):
            ...
            if i == 4:
                deconv1 = layers.conv2d_transpose(curr_layer, W=[3, 3, num_filter // 2, num_filter // 2], b=[num_filter // 2], output_shape=tf.shape(conv_layer_list[i]), stride=1)
        ...
        conv = layers.conv2d(curr_layer, W=[5, 5, base_filter_num, 3], b=[3])
        # relu = tf.nn.relu(deconv)
        # print('final relu: ', relu.get_shape().as_list())

        print('trainable: ', tf.trainable_variables())
        return tf.expand_dims(tf.argmax(conv, axis=-1), axis=-1), conv
